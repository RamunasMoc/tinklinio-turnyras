    ...
    <div>
      {isKO && (
        <div className="mb-4 rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div className="grid w-full grid-cols-2 rounded-lg bg-gray-100 p-1 sm:inline-flex sm:w-auto" role="group" aria-label="KO nugalėtojo parinkimo režimas">
            <button
              type="button"
              onClick={() => chooseKOWinnerMode('ranked')}
              aria-pressed={koWinnerMode === 'ranked'}
              className={`rounded-md px-2 py-2 text-xs font-medium transition-colors sm:px-3 sm:text-sm ${koWinnerMode === 'ranked' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-800'}`}
            >
              Pagal reitingą
            </button>
            <button
              ...
              className={`rounded-md px-2 py-2 text-xs font-medium transition-colors sm:px-3 sm:text-sm ${koWinnerMode === 'random' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-800'}`}
            >
        ...
        <div className="grid w-full grid-cols-1 gap-2 sm:ml-auto sm:flex sm:w-auto sm:flex-wrap sm:items-center sm:justify-end">
          ...
          <form
            ...
            <button type="submit" disabled={filling}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600 disabled:opacity-50 sm:w-auto">
              {filling ? <><span className="animate-spin inline-block">⟳</span> Užpildoma...</> : <>🎲 Užpildyti atsitiktinai ({randomFillCount})</>}
            </button>
          </form>
          )}
          {pending.length === 0 && <span className="text-sm font-medium text-green-600">✓ Visi rezultatai įvesti</span>}
          ...
          <form
            ...
            <input type="hidden" name="isKO" value={isKO ? 'true' : 'false'} />
            <button type="submit" disabled={filling}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-red-500 px-4 py-2 text-sm font-medium text-white hover:bg-red-600 disabled:opacity-50 sm:w-auto">
              🗑 Išvalyti visus
            </button>
          </form>
          )}
        ...
        {filtered.map(m => {
          ...
          return (
            <div key={m.id}
              ...
              <div className="grid grid-cols-[3.5rem_minmax(0,1fr)] items-start gap-x-3 gap-y-2 sm:flex sm:items-center sm:gap-3">
                {/* Kairė: aikštelė + laikas + etapas */}
                <div className="row-span-2 w-14 shrink-0 cursor-pointer text-center" onClick={()=>openMatch(m)}>
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={()=>openMatch(m)}>
                  <div className={`-mx-1 flex min-w-0 items-baseline rounded px-1 text-sm font-medium ${hWin?'text-green-700 bg-green-50':aWin?'text-gray-400':'text-gray-900'}`}>
                    ...
                    <span className="min-w-0 truncate">{m.homeTeam?.team.name}</span>
                    {m.homeTeam && ((m as any).homeTeam?.groupWins > 0 || (m as any).homeTeam?.groupLosses > 0) && (
                      <span className="ml-1.5 hidden shrink-0 text-xs font-normal text-gray-400 sm:inline">
                        {(m as any).homeTeam.groupWins}L/{(m as any).homeTeam.groupLosses}P · {(m as any).homeTeam.groupPoints}t.
                      </span>
                    )}
                  </div>
                  <div className={`-mx-1 mt-0.5 flex min-w-0 items-baseline rounded px-1 text-sm ${aWin?'text-green-700 font-medium bg-green-50':hWin?'text-gray-400':'text-gray-600'}`}>
                    ...
                    <span className="min-w-0 truncate">{m.awayTeam?.team.name}</span>
